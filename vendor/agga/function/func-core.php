...
function parserlink($str,$site,$reg,$link,$img,$intro,$req){
    ...
    foreach ($blog as $res){
       ...
       // }
            ...
            if(strpos($links,$site_domain)==1){     //博文链接与站点域名匹配
                ...
                //echo '2'.strpos($links,$site_domain);
                $links = $bloglink;
            }else{
                //echo '3"'.strpos($links,$site_domain).'"|"'.stripos($links,'http');
                $links = $site_domain.$bloglink;
                
            }
        //echo $links.' | '.$site_domain.' | '.$bloglink.' | ';
		//$intro = 
